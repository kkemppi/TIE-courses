...
class Car:
    ...
    def fill(self, to_fill):
        if to_fill < 0:
            ...
            self.__gas = float(self.__tank_size)
        else:
    ...
    def drive(self, distance):
        if distance < 0:
            ...
            return
        gas_per_km = self.__gas_consumption / 100
        gas_needed = distance * gas_per_km
        max_dist = self.__gas_consumption * self.__gas
        if gas_needed <= self.__gas:
            self.__odometer += distance
            self.__gas -= gas_needed
        else:
            self.__odometer += max_dist
            self.__gas = 0
