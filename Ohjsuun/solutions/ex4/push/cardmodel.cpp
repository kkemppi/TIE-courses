...
CardModel::CardModel(std::shared_ptr<CardView> view) :
    view_{ view },
    cardValue_{ CARDS[0] },
...
void CardModel::setCardChoice(const QString& choice)
{
    ...
    if (playerChoice == PlayerChoice::SMALL || playerChoice == PlayerChoice::BIG)
    {
        ...
        if (correctChoice_)
        {
            ...
            if (view_ != nullptr)
            {
                view_->setCorrectAnswers(correctAnswers_);
            }
        }
        else
        {
            ...
            if (view_ != nullptr)
            {
                view_->setIncorrectAnswers(incorrectAnswers_);
            }
        }

        if (view_ != nullptr)
        {
            view_->setGuessCorrect(correctChoice_);
        }
    }
}
