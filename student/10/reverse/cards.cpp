...
void Cards::reverse()
{
    std::shared_ptr <Card_data> top_of_new_deck = nullptr;

    while(top_ != nullptr){
        std::shared_ptr <Card_data> top_of_old_deck = top_;
        top_ = top_ -> next;

        top_of_old_deck -> next = top_of_new_deck;
        top_of_new_deck = top_of_old_deck;

    }
    top_ = top_of_new_deck;
}
